 ...
 */
export function parseWallValue(text: string): number {
    if (!text) return 0;
 ...
 */
export function calculateNeededTroopsFromWallValue(wallValue: number, bonusPercent: number): number {
...
export function createEmbed(title: string, description: string, color = 0x00ADEF) {
...
export function calculatePower(troops: number, bonusPercent: number): number {
